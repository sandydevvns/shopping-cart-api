...
func AdminCategoryList(c *gin.Context) {
	...
	rows, _ := database.Raw("select distinct cat.id, cat.name, cat.description,cat.parent_id, f.filename,f.file_path, pcat.name as parent from categories as cat " +
		"left join file_uploads as f on f.category_id = cat.id " +
		"left join categories as pcat on pcat.id = cat.parent_id order by cat.id desc").Rows()
